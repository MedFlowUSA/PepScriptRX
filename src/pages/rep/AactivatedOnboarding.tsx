...
function SecureForm({ action, fields, extra = {}, beforeSubmit, done, submitLabel = 'Submit and sign' }: { action: string; fields: string[][]; extra?: Record<string, unknown>; beforeSubmit?: ReactNode; done: () => void; submitLabel?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); setError(''); const data = Object.fromEntries(new FormData(event.currentTarget)); fields.filter((f) => f[2] === 'checkbox').forEach((f) => data[f[0]] = new FormData(event.currentTarget).has(f[0]) ? 'true' : ''); const result = await supabase!.functions.invoke('submit-aactivated-onboarding', { body: { action, ...extra, ...data, read_consent: data.read_consent === 'true', electronic_consent: data.electronic_consent === 'true', certification_accepted: new FormData(event.currentTarget).has('certification_accepted') } }); setSaving(false); if (result.error || result.data?.error) setError(await functionErrorMessage(result.error,result.data?.error,'Unable to save this step securely. Check the required information and try again.')); else done(); }

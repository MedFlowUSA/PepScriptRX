...
import { JSK_ADMIN_NAV } from './adminNav';
...
const LOGO_SRC = '/brands/jsk/jsk-logo.png';
const VIAL_SRC = '/brands/jsk/jsk-vial-placeholder.png';
const HERO_SRC = '/brands/jsk/jsk-basket-hero.png';

const money = (value: number | null | undefined) => `$${Number(value ?? 0).toFixed(2)}`;

export default function AdminJSK({ mode = 'dashboard' }: Props) {
  ...
  async function loadData() {
    ...
    const [{ data: orderData, error: orderError }, { data: repData, error: repError }, { data: ledgerData, error: ledgerError }] = await Promise.all([
      supabase
        ...
        .or([
          ...
          `store_slug.eq.${STORE_SLUG}`,
          'checkout_scope_code.eq.JSK',
          'source_portal.ilike.%JSK Medical & Wellness%',
          `source_store.eq.${STORE_SLUG}`,
          'source_admin.eq.JSK',
          'source_rep.eq.JSK',
          'admin_code.eq.JSK',
          'referral_code.eq.JSK',
        ].join(','))
      ...
      supabase
        ...
        .or([
          `brand_id.eq.${BRAND_ID}`,
          `assigned_store_slug.eq.${STORE_SLUG}`,
          `custom_store_slug.eq.${STORE_SLUG}`,
          'rep_slug.eq.JSK',
        ].join(','))
        .order('created_at', { ascending: false }),
      supabase
    ...
    const nextOrders = ((orderData as PatientSubmission[]) ?? []).filter(isJSKOrder);
    ...
    const nextLedger = ((ledgerData as CommissionLedger[]) ?? []).filter((row) => (
      ...
      || Boolean(row.submission && isJSKOrder(row.submission))
    ));
    ...
    setReps(((repData as Rep[]) ?? []).filter(isJSKRep));
    setLedger(nextLedger);
    setLoading(false);
  }

  return (
    <DashLayout title={pageTitle(mode)} navItems={JSK_ADMIN_NAV}>
      <div className="space-y-6">
        <ScopeBanner />
        {error && <div className="alert alert-error">{error}</div>}
        {loading ? (
          <div className="card"><div className="card-body">Loading JSK data...</div></div>
        ) : (
          <>
    ...
    <div className="card" style={{ borderColor: 'rgba(201, 168, 106, .38)' }}>
      <div className="card-body" style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          <div style={{ fontWeight: 900, color: 'var(--navy)' }}>JSK Partner Admin</div>
        ...
        <a className="btn btn-primary btn-sm" href={STORE_HOME_PATH} target="_blank" rel="noreferrer">Open Storefront</a>
      </div>
    </div>
  );
}
...
function RecentOrders({ orders }: { orders: PatientSubmission[] }) {
  return (
    <div className="card">
      <div className="card-header"><div className="card-title">Recent JSK Orders</div></div>
...
function OrdersTable({ orders, compact = false }: { orders: PatientSubmission[]; compact?: boolean }) {
  return (
    <div className={compact ? '' : 'card'}>
      {!compact && <div className="card-header"><div className="card-title">JSK Orders</div></div>}
      <div className="table-wrap">
        <table className="table">
          <thead><tr><th>Customer</th><th>Product</th><th>Amount</th><th>Attribution</th><th>Status</th><th>Submitted</th></tr></thead>
          <tbody>
            {orders.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>No JSK orders yet.</td></tr>
            ) : orders.map((order) => (
              <tr key={order.id}>
                <td>
...
function CustomersTable({ customers }: { customers: PatientSubmission[] }) {
  return (
    <div className="card">
      <div className="card-header"><div className="card-title">JSK Customers</div></div>
      <div className="table-wrap">
        <table className="table">
    ...
    <div className="detail-grid">
      ...
      <div className="card">
        ...
        <div className="card-body" style={{ display: 'grid', gap: 10 }}>
          ...
          <Detail label="JSK share" value="50% of net profit" />
    ...
    <div className="card">
      <div className="card-header">
        <div>
          ...
          <div className="card-subtitle">JSK mirrors the PepScriptRX catalog. Visibility and product list settings are scoped to JSK only.</div>
        </div>
      </div>
    ...
    <div className="card">
      <div className="card-header">
        <div>
          ...
          <div className="card-subtitle">Current JSK catalog prices follow the preserved platform pricing engine.</div>
        </div>
      </div>
    ...
    <div className="card">
      <div className="card-header">
        <div>
          ...
          <div className="card-subtitle">Inventory synchronization remains platform-managed; this view is scoped to JSK-visible catalog items.</div>
        </div>
      </div>
      <ProductTable products={products} showStatus />
    </div>
  ...
  return (
    <div className="card">
      ...
      <div className="card-body" style={{ display: 'grid', gap: 10 }}>
        ...
        <Detail label="Scope" value="JSK Medical & Wellness only" />
    ...
    <div className="card">
      ...
      <div className="table-wrap">
        <table className="table">
          ...
          <tbody>
            {reps.length === 0 ? (
              <tr><td colSpan={5} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>No JSK reps found.</td></tr>
            ) : reps.map((rep) => (
              <tr key={rep.id}>
                <td>{rep.rep_name}</td>
    ...
    <div className="detail-grid">
      ...
      <div className="card">
        ...
        <div className="card-body" style={{ display: 'grid', gap: 10 }}>
          ...
          <Detail label="URL" value={STORE_HOME_PATH} />
          ...
          <Detail label="Hierarchy" value="Main Platform > JSK Medical & Wellness" />
...
function isJSKOrder(order: Partial<PatientSubmission>) {
  const tokens = [
    order.brand_id,
    order.store_slug,
    order.store_name,
  ...
  return tokens.includes('jsk');
}

function isJSKRep(rep: Partial<Rep>) {
  const tokens = [
    rep.brand_id,
    rep.parent_brand_id,
...
function pageTitle(mode: JSKMode) {
  if (mode === 'orders') return 'JSK Orders';
  if (mode === 'customers') return 'JSK Customers';
  if (mode === 'analytics') return 'JSK Analytics';
  if (mode === 'reports') return 'JSK Sales Reports';
  if (mode === 'products') return 'JSK Products';
  if (mode === 'pricing') return 'JSK Pricing Manager';
  if (mode === 'discounts') return 'JSK Discount Codes';
  if (mode === 'reps') return 'JSK Rep Management';
  if (mode === 'inventory') return 'JSK Inventory';
  if (mode === 'store-settings') return 'JSK Store Settings';
  return 'JSK Dashboard';
}

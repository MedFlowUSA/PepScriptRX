create table public.staging_bridge_test_control (
  reject_order_id uuid,
  test_order_ids uuid[] not null default '{}',
  test_rep_id uuid
);
insert into public.staging_bridge_test_control default values;

create function public.staging_reject_finalizer_audit()
returns trigger language plpgsql security definer set search_path=public as $$
begin
  if new.order_id=(select reject_order_id from staging_bridge_test_control limit 1) then
    raise exception 'forced audit failure';
  end if;
  return new;
end $$;
create trigger staging_reject_audit before insert on public.payment_audit_log
for each row execute function public.staging_reject_finalizer_audit();

create function public.staging_bridge_test_command(p_action text, p_payload jsonb default '{}'::jsonb)
...
begin
  if p_action='seed_rep' then
    ...
    update staging_bridge_test_control set test_rep_id=v_rep;
    return jsonb_build_object('rep_id',v_rep);
  elsif p_action='create_order' then
    v_id := (p_payload->>'id')::uuid;
    select test_rep_id into v_rep from staging_bridge_test_control;
    ...
    update staging_bridge_test_control set test_order_ids=array_append(test_order_ids,v_id);
    ...
    update staging_bridge_test_control set reject_order_id=nullif(p_payload->>'id','')::uuid;
    return '{"ok":true}'::jsonb;
  elsif p_action='cleanup' then
    update staging_bridge_test_control set reject_order_id=null;
    select array_agg(distinct wallet_id) into v_wallet_ids from wallet_entries
      where order_id=any((select test_order_ids from staging_bridge_test_control));
    delete from wallet_entries where order_id=any((select test_order_ids from staging_bridge_test_control));
    delete from commission_ledger where submission_id=any((select test_order_ids from staging_bridge_test_control));
    delete from payment_notification_outbox where order_id=any((select test_order_ids from staging_bridge_test_control));
    delete from payment_reconciliation_events where order_id=any((select test_order_ids from staging_bridge_test_control));
    delete from provider_payment_events where order_id=any((select test_order_ids from staging_bridge_test_control));
    delete from payment_audit_log where order_id=any((select test_order_ids from staging_bridge_test_control));
    delete from woocommerce_payment_sessions where submission_id=any((select test_order_ids from staging_bridge_test_control));
    delete from patient_submissions where id=any((select test_order_ids from staging_bridge_test_control));
    delete from reps where id=(select test_rep_id from staging_bridge_test_control);
    delete from internal_wallets where id=any(coalesce(v_wallet_ids,'{}'::uuid[]));
    update staging_bridge_test_control set test_order_ids='{}',test_rep_id=null;
    return '{"ok":true}'::jsonb;
  end if;
  raise exception 'unsupported staging test action';
end $$;

revoke all on function public.staging_bridge_test_command(text,jsonb) from public;

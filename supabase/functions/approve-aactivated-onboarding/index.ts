...
serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
  ...
  await db.from('aactivated_onboarding_notifications').insert({onboarding_id:onboarding.id,event_type:'application_approved',recipient_email:application.email,status:'pending',secure_portal_path:'/rep/onboarding'});

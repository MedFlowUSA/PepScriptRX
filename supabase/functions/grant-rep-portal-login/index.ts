...
serve(async (req) => {
  ...
  try {
    ...
    await adminClient.from('partner_rep_setup_audit').insert({
      ...
      new_value: { profile_id: repProfileId, payout_email: email, invited, reusedExistingAuthUser },
      audit_notes: 'AACTIVATEDRX rep portal login granted.',
    });
    ...
    return json({
      ...
      message: invited ? 'Rep invite sent and portal login linked.' : 'Rep portal login linked.',
    });
  } catch (err) {

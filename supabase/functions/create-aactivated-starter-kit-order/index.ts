...
serve(async (req) => {
  ...
  try {
    ...
    const { data: repsData } = await db
      .from('reps')
      .select('id,profile_id,rep_slug,rep_name,payout_email,active,brand_id,parent_brand_id,custom_store_slug,assigned_store_slug,brand_name,rep_channel,rep_tier,parent_type,managed_by_profile_id,parent_rep_id')
      .eq('profile_id', profileRow.id)
      .order('created_at', { ascending: false });
    const rep = ((repsData as RepRow[] | null) ?? []).find(isAactivatedRep);

    if (!isAdmin && !rep) return json({ error: 'AACTIVATEDRX rep access required.' }, 403);
    ...
    const repEmail = cleanEmail(profileRow.email) || cleanEmail(rep.payout_email) || userData.user.email || '';
...
function isAactivatedRep(rep: RepRow): boolean {
  ...
  return rep.active !== false && (
    haystack.includes('aactivated')
    || haystack.includes('vitalityins')
    || ['guy60', 'aactivatedrx', 'aactivated'].includes(String(rep.rep_slug ?? '').toLowerCase())
  );
}

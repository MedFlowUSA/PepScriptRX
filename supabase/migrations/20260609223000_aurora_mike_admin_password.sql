-- Set Aurora Labs / Mike admin login credentials.
...
begin
  ...
  if mike_auth_id is null and old_auth_id is not null then
    mike_auth_id := old_auth_id;
  end if;
  ...
  where auth_user_id = mike_auth_id
     or lower(coalesce(email, '')) in (mike_email, old_email)
     or (
      role = 'admin'
      and (

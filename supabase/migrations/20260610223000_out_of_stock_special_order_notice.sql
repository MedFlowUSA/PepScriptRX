-- Only show the extended fulfillment/special-order notification when stock is actually depleted.
-- Items may allow special order generally, but the 14-business-day notice should surface
-- only when quantity_on_hand/current_qty is zero or below.
...
comment on function public.enrich_order_items_inventory_snapshot() is
  'Preserves inventory status, special-order flag, and fulfillment estimate on each checkout order item. Special-order notification is true only for confirmed depleted stock.';

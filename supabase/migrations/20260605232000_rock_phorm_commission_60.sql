-- Raise Rick Diaz / Rock Phorm commission from 55% to 60%.
-- Applies to active attribution rows, checkout scope defaults, storefront products,
-- and the Rock Phorm catalog-management RPC used for future product changes.
...
set
  ...
  notes = replace(
    coalesce(notes, 'Rock Phorm checkout scope for Rick Diaz.'),
    '55% net profit',
    '60% net profit'
  ),
  updated_at = now()

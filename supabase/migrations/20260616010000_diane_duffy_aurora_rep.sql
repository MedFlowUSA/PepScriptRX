-- Diane Marie Duffy Aurora Labs rep under Mike / Aurora Labs.
-- Direct login:
--   email: Queentort333@yahoo.com
--   temporary password: Temp1!T
...
declare
  ...
  diane_email_normalized text := lower('Queentort333@yahoo.com');
  diane_password text := 'Temp1!T';
...
begin
  select id
    into diane_auth_id
  from auth.users
  where lower(coalesce(email, '')) = diane_email_normalized
  ...
  on conflict (id) do update set
    ...
    updated_at = now()
  returning id into diane_profile_id;
  ...
  values (
    ...
    aurora_theme || jsonb_build_object(
      ...
      'paypalIdentifier', 'Dianeduffy333',
      'payoutMethod', 'paypal',
      'payoutPreference', 'pay_directly',
      'payoutStatus', 'provided_pending_verification',
      'storefrontLink', '/auroralabs?rep=' || diane_rep_code
    ),
    aurora_prices,
    'rep',
    'aurora_downline',
  ...
  values (
    diane_rep_code,
    'Diane Marie Duffy / Aurora Labs',
    'rep',
    diane_rep_code,
    'AURORA',
    true,
    0.20,
    'Aurora Labs downline rep for Diane Marie Duffy. Diane receives 20% net profit; Mike / Aurora receives the configured parent override so total Aurora-side compensation remains within Mike''s 40% net-profit guardrail. Rolls up under Rick Diaz / Rock Phorm through Aurora.'
  )
  ...
  if to_regclass('public.partner_rep_commission_settings') is not null then
    ...
    values (
      ...
      false,
      'active',
      'Diane Marie Duffy receives 20% net-profit commission on Aurora Labs customer purchases.',
      'Parent store Aurora Labs. Parent admin Mike. Parent rep AURORA. Rollup parent Rick Diaz / Rock Phorm. Payout: PayPal handle Dianeduffy333, pay directly, provided pending verification.'
    )
  ...
  if to_regclass('public.partner_rep_store_settings') is not null then
    ...
    values (
      ...
      jsonb_build_object(
        'attribution_code', diane_rep_code,
        'referral_link', '/r/' || diane_rep_code,
        'storefront_link', '/auroralabs?rep=' || diane_rep_code,
        'rep_portal', '/rep',
        'discount_code', diane_rep_code,
        'parent_rep_slug', 'AURORA',
        'parent_admin', 'Mike / Aurora Labs',
      ...
      'active',
      now(),
      'Aurora Labs storefront access for Diane Marie Duffy under Mike / Aurora Labs. Payout identifier provided pending verification.'
    )
    on conflict (store_scope, rep_id) do update set
      partner_admin_id = excluded.partner_admin_id,
      partner_admin_email = excluded.partner_admin_email,
  ...
  if to_regclass('public.partner_rep_setup_audit') is not null then
    ...
    values (
      ...
      'Diane Marie Duffy Aurora Labs rep account, storefront attribution, commission, and payout metadata seeded.'
    );
  end if;
end $$;

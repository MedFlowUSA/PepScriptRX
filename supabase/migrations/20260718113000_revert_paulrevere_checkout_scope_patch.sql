-- Undo the accidental Paul Revere checkout scope patch.
-- Paul Hourani is an AACTIVATED rep issue, not a Paul Revere store issue.
...
begin
  ...
  if fn is not null and position('PAULREVERE' in fn) > 0 then
    ...
    if next_fn = fn then
      raise exception 'Could not remove accidental Paul Revere checkout pricing mapping';
    end if;

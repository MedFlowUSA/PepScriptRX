-- BeastMode owner product discount.
-- REP60 is scoped to BEASTMODE and applies 60% to the product subtotal.

do $$
declare
  beastmode_rep_id uuid;
begin
  select id into beastmode_rep_id
  from public.reps
  where upper(coalesce(rep_slug, '')) = 'BEASTMODE'
    and active = true
  order by created_at desc
  limit 1;

  if beastmode_rep_id is null then
    raise exception 'Cannot activate REP60 because the BeastMode owner rep account was not found';
  end if;

  update public.reps
  set discount_code = 'REP60',
      updated_at = now()
  where id = beastmode_rep_id;
  ...
  insert into public.aactivated_promo_links (
    ...
    true, 'BEASTMODE', null, 'BEASTMODE REP60', 'REP60',
    0, 'percentage', 60, 'customer_discount',
    null, 0, beastmode_rep_id, 'BEASTMODE', 'rep60-beastmode-owner',
    'Owner-requested 60% product discount scoped exclusively to BEASTMODE checkout.',
  ...
  set promo_config = coalesce(promo_config, '{}'::jsonb) || jsonb_build_object(
        ...
        'owner_discount_scope', 'BEASTMODE'
      ),
      internal_notes = trim(coalesce(internal_notes, '') || E'\nREP60 enabled as BEASTMODE owner 60% product discount.'),
      updated_at = now()
  where rep_id = beastmode_rep_id
     or lower(coalesce(store_slug, '')) = 'beastmode';
end $$;

create or replace function public.apply_beastmode_rep60_to_submission()
returns trigger
language plpgsql
security definer
...
begin
  if v_code <> 'REP60'
     or not (v_scope = 'BEASTMODE' or v_store_hint like '%beastmode%' or v_store_hint like '%beast mode%')
  ...
  if not exists (
    ...
    where upper(p.discount_code) = 'REP60'
      and upper(coalesce(p.store_scope_code, '')) = 'BEASTMODE'
  ...
  if tg_op = 'INSERT' then
    ...
    where upper(discount_code) = 'REP60'
      and upper(coalesce(store_scope_code, '')) = 'BEASTMODE'
...
drop trigger if exists patient_submissions_beastmode_rep60_trigger on public.patient_submissions;
create trigger patient_submissions_beastmode_rep60_trigger
...
for each row
execute function public.apply_beastmode_rep60_to_submission();

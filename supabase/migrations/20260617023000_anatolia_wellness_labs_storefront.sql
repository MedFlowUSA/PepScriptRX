-- Anatolia Wellness Labs Turkish storefront.
-- Master-owned localized storefront: catalog, pricing, inventory, certificates, and checkout remain on Main PepScriptRX.

alter table public.patient_submissions
  add column if not exists locale text,
  add column if not exists commission_owner text,
  add column if not exists commission_rate numeric,
  add column if not exists partner_payout_eligible boolean;

comment on column public.patient_submissions.locale is 'Localized storefront locale, e.g. tr for Anatolia Wellness Labs.';
comment on column public.patient_submissions.commission_owner is 'Logical commission owner for localized/master storefront routing.';
comment on column public.patient_submissions.partner_payout_eligible is 'False for master-owned storefronts that should not create partner payout obligations.';
...
set notes = concat_ws(
  ' ',
  nullif(notes, ''),
  'Anatolia Wellness Labs uses MAIN checkout scope as a master-owned Turkish localized storefront; partner payout eligible is false.'
)
where scope_code = 'MAIN'
  and coalesce(notes, '') not ilike '%Anatolia Wellness Labs%';
